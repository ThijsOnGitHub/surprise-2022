<template>
  <div v-if="role==='admin'">
    <p>Hier komt scene 2</p>
    <NextSceneButton :restore-state="false"/>
  </div>
  <div v-else-if="role === 'screen'" class="black__screen  " >
    ...
    <div class="centerContent call__content">
      <img class="sint__foto" src="@/assets/2021/profielfoto_sint.jpg"/>
      <div class="icon__bar">
        <div class="icon__circle" :style="{backgroundColor: mutedIcon ? 'red':'white'}" >
          <img v-if="mutedIcon" src="@/assets/2021/icons/mute_mic.svg"/>
          <img v-else src="@/assets/2021/icons/mic.svg"/>
        </div>
        <div class="icon__circle">
          <img src="@/assets/2021/icons/mute_video.svg"/>
        </div>
        <div class="icon__circle">
          <img src="@/assets/2021/icons/flip_cam.svg"/>
        </div>
      </div>
    </div>
...
export default class scene2 extends Vue {
  ...
  sound = new Audio(require('@/assets/2021/audio/sint_muted.mp3'))
  seconds=0.0
  mutedIcon=false
  timeInterval = 0
